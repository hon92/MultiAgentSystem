...
public class ReceiveAction extends Action
{
    ...
    private final String PATTERN = "(receive)\\s(\\d{0,3}.\\d{0,3}.\\d{0,3}.\\d{0,3})\\s(\\d+)\\s(.+)";

    public ReceiveAction(String prefix, int paramsLength, Agent agent)
    {
    ...
    public void perform(String msg) throws Exception
    {
        ...
        if (m.find() && m.groupCount() == getParamsLength() + 1)
        {
            ...
            boolean sended = sendMessageToAddress("ack " + getAgent().getIp() + " " + getAgent().getPort(), ip, port);
            if (!sended)
            {
                throw new Exception("Cant send socket to address " + ip + ":" + portStr);
            }
            getAgent().addMessage(message);
        }
        else
        {
